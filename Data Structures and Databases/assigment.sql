...
CREATE TABLE kredikartları (
  kartNumarası VARCHAR(45) PRIMARY KEY,
  kullanıcıID INT NOT NULL,
  isimSoyisim VARCHAR(45) NOT NULL,
  sonKullanma VARCHAR(45) NOT NULL,
  CVV VARCHAR(45) NOT NULL,
  Foreign Key (kullanıcıID) REFERENCES kullanıcıbilgileri(kullanıcıID)
);
...
--2nd Query
SELECT kullanıcıbilgileri.*, oteller.*
FROM kullanıcıbilgileri
